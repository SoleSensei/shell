...
tree *build_tree(char **s) //building tree of cmd
{
    ...
    if (!q)
    {
        fprintf(stderr, "tree: tree calloc err\n");
        return q;
    }
    ...
    for (i = 0; ex && s[o]; ++o, ++i)
    {
        ...
        default:
            ...
            {
                fprintf(stderr, "tree: argv realloc err\n");
                return q;
            }
            q->argv[i] = s[o];
            q->argv[i + 1] = NULL;
                ...
                if ((pid = fork()) == 0) //grand child
                {
                    ...
                    while (t->pipe)
                    {
                        ...
                        if ((pid2 = fork()) == 0) //stdout -> stdin
                        {
                            ...
                            close(pd[0]);
                            close(pd2[0]); //close child's descr
                            close(pd[1]);
                            close(pd2[1]);
                            execvp(*t->argv, t->argv);
                            perror("exec error");
...
char *get_env(char **str) //get environment
{
    ...
    else if (!strncmp(*str, "$USER", env_s))
    {
        buf = getenv("USER");
        env = (char *)malloc(strlen(buf) * sizeof(char));
        strcpy(env, buf);
    }
    else if (!strncmp(*str, "$EUID", env_s))
    {
        env = (char *)malloc(10 * sizeof(char));
        sprintf(env, "%d", geteuid()); //int -> str
    }
    else if (!strncmp(*str, "$SHELL", ++env_s))
    {
        buf = getenv("SHELL");
        env = (char *)malloc(strlen(buf) * sizeof(char));
        strcpy(env, buf);
...
char *add_str(int num, char c) //num of sym, sym | forming string with spec symbol
{
    ...
    if (str == NULL)
    {
        fprintf(stderr, "add_str: malloc err\n");
        exit(1);
    }
    memset(str, c, num);
    str[num] = '\0';
...
char **form_list(char *str) //forming list from string
{
    ...
    if (res == NULL)
    {
        fprintf(stderr, "form_list: res malloc err\n");
        exit(1);
    }
    while (*str)
    {
        ...
        if (wd == NULL)
        {
            fprintf(stderr, "form_list: wd realloc err\n");
            exit(1);
        }
        wd[k++] = *str++;
        wd[k] = '\0'; //eol
    ...
    if (quo)
        fprintf(stderr, "form_list: quotes were not closed\n");
